...
const FoodCard = React.memo(({ item, onClick }) => {
  ...
  const itemData = useMemo(() => {
    ...
    // Получаем путь к изображению напрямую без дополнительных преобразований
    const rawImagePath = item?.image || item?.images || '';
    const imagePath = getImagePath(rawImagePath) || PLACEHOLDER_IMAGE;
  ...
  return (
    ...
    >
      {/* Image Container - простая загрузка без задержек */}
      <div className="relative aspect-video overflow-hidden bg-gray-100">
        ...
        {/* Градиент */}
        <div className="absolute inset-0 bg-gradient-to-t from-black/30 to-transparent opacity-50 pointer-events-none"></div>
        ...
        {/* Метка специального предложения */}
